...
namespace ch8 {
	...
	void Commands::runCommands(CPU* chip) {
		...
		std::cout << "INS: " << rdx::toHex(instruction) << "\n";
		...
		case 0x0000:
			...
			if (instruction == 0x00EE)RET_00EE(chip);
			break;
		case 0x1000:
			JMP_1nnn(chip);
			break;
...
#pragma endregion	
	...
	void Commands::RET_00EE(CPU* chip) {
	...
	void Commands::CALL_2nnn(CPU* chip){
		std::cout << "CALL \n";
		chip->stack[chip->stackPointer] = chip->programCounter; // stack = pc
		chip->stackPointer += 1;
